...
<div class="max-w-full sm:min-w-96">
	<div
		class="flex flex-col border-2 border-[--custom-light-pale] bg-[--custom-light-pale] shadow-lg rounded-xl overflow-hidden"
	>
		<h2 class="font-extrabold text-sm p-2 text-center">AMBITION DETAILS</h2>
		<div class="bg-background rounded-lg space-y-10 p-4">
			<ul class="space-y-5">
				<li class="flex justify-between gap-16 w-full border-b py-1">
				...
				<li class="flex justify-between gap-16 w-full border-b py-1">
					...
					<p class="flex place-items-center gap-2">
						...
						{:else if ambitionData.ambitionPriority.toUpperCase() === 'LOW'}
							<span class="border border-green-500 px-1 rounded-md text-green-500 text-sm">!</span>
						{/if}
						<span>
